...
> Source: https://www.freecodecamp.org/learn/javascript-v9/lecture-working-with-strings-in-javascript/how-can-you-find-the-position-of-a-substring-in-a-string · Saved 8/25/2026, 5:02:32 PM
...
- If the substring is found, `indexOf()` returns the index of the first occurrence.
- If the substring is not found, `indexOf()` returns `-1`.
- `indexOf()` can take an optional second argument to start searching from a specific index.
- `indexOf()` is case-sensitive, so uppercase and lowercase letters must match exactly.

## What the MCQs are asking
- What does `indexOf()` return when the substring is not found?
- How can you use `indexOf()` to start searching from a specific position in the string?
- What will `str.indexOf("Javascript")` return when `str = "I am learning JavaScript."`?

## What it means
- A return value of `-1` means the substring was not found.
- The second argument to `indexOf()` tells JavaScript where to begin searching, but the returned index is still the position in the original string.
- The third question tests case sensitivity: `"Javascript"` does not exactly match `"JavaScript"`, so the result is `-1`.

## Most important things to know
- `indexOf()` returns the index of the first match only.
- Use `indexOf(substring, startIndex)` to search from a specific position.
