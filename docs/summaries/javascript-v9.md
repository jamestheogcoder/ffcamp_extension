...
> Source: https://www.freecodecamp.org/learn/javascript-v9/#lecture-working-with-strings-in-javascript · Saved 8/25/2026, 5:04:29 PM
...
- `prompt()` is a JavaScript method that opens a small browser dialog box to ask the user for text input.
- It takes two arguments: `prompt(message, default)`.
  - `message`: the text shown in the dialog box.
  - `default`: an optional value that pre-fills the input field.
- It returns the user’s entered text as a string.
- If the user cancels the dialog, it returns `null`.
- It pauses script execution until the user clicks “OK” or “Cancel.”
- It is simple but often avoided in modern web apps because it can be intrusive and behaves inconsistently across browsers.

## What the MCQs are asking
- First question: What does `prompt()` do?
  - It asks the user for input and returns the entered text.
- Second question: What happens if the user cancels the prompt?
  - The method returns `null`.
- Third question: What is the second, optional argument used for?
  - It sets the default value shown in the input field.

## What it means
- `prompt()` is a basic way to collect information from a user, such as a name.
- The returned value can be stored in a variable and used later, like in a greeting message.
- `null` means the user did not provide input, so code should handle that case to avoid displaying “null.”
- Because it blocks execution, the rest of the script waits until the user responds.

## Most important things to know
- Syntax: `prompt(message, default)`.
- Return value:
  - String if the user enters text and clicks “OK.”
  - `null` if the user cancels.
- The second argument is optional and only pre-fills the input field.
- `prompt()` stops the script until the user responds.
- It is useful for learning and simple demos, but not ideal for modern web applications.
